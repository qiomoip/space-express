...
void CEditorScene::Input()
{
	...
	if(pInfo->bPush)
	{
		...
		if(pTerrainMesh->GetCollisionPos(tRay))
		{
			...
			float fHeight = ((CTerrainMesh*)_SINGLE(CObjectManager)->FindObject("MainTerrain")->GetMesh())->GetHeight(tRay.vIntersectPos.x, tRay.vIntersectPos.z);
			pBox->SetPos(D3DXVECTOR3(tRay.vIntersectPos.x, fHeight, tRay.vIntersectPos.z));
			//pBox->SetScale(1, 1, 1);
